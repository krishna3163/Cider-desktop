...
const app = new Vue({
  ...
  methods: {
    ...
    async getCurrentArtURL() {
      ...
      let mediaItem = null;
      if (app?.mk?.nowPlayingItem?.attributes?.artwork?.url) {
        mediaItem = app.mk.nowPlayingItem;
      } else if (this.mk?.nowPlayingItem?.id) {
        try {
          const res = await this.mk.api.v3.music(`/v1/me/library/songs/${this.mk.nowPlayingItem.id}`);
          if (res?.data?.data?.[0]) {
            mediaItem = res.data.data[0];
          }
        } catch (e) {
          // Gracefully catch 404s for catalog/non-library tracks
        }
      }
      ...
      const rawUrl = mediaItem?.attributes?.artwork?.url ?? "";
      const artUrl = rawUrl ? rawUrl.replace("{w}", artworkSize).replace("{h}", artworkSize) : (mediaItem?._assets?.[0]?.artworkURL ?? "");
