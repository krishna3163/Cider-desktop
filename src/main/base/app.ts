...
export class AppEvents {
  ...
  private start(): void {
    ...
    // Limit JS memory to 1024MB to prevent constant GC stutters while keeping memory low.
    ...
    switch (utils.getStoreValue("visual.hw_acceleration") as string) {
      ...
      case "disabled":
        if (process.platform === "win32") {
          console.warn("[AppEvents] Hardware acceleration was requested to be disabled, but was forced enabled because Widevine DRM requires GPU.");
          app.commandLine.appendSwitch("enable-accelerated-mjpeg-decode");
          app.commandLine.appendSwitch("enable-accelerated-video");
          app.commandLine.appendSwitch("disable-gpu-driver-bug-workarounds");
          app.commandLine.appendSwitch("ignore-gpu-blacklist");
