...
export class LocalFiles {
  ...
  static getCachePaths() {
    ...
    const cacheDir = join(app.getPath("userData"), "CiderCache");
    return {
      songsPath: join(cacheDir, "local-songs-cache.json"),
      artsPath: join(cacheDir, "local-songs-arts-cache.json"),
    };
  }
  ...
  static loadFromCache() {
    try {
      ...
      if (existsSync(paths.songsPath)) {
        this.localSongs = JSON.parse(readFileSync(paths.songsPath, "utf-8"));
      }
      if (existsSync(paths.artsPath)) {
        this.localSongsArts = JSON.parse(readFileSync(paths.artsPath, "utf-8"));
      }
      console.log(`[LocalFiles] Loaded ${this.localSongs.length} local songs from cache.`);
    } catch (e) {
      console.error("[LocalFiles] Failed to load local songs from cache:", e);
    }
  }
  ...
  static saveToCache() {
    try {
      const paths = this.getCachePaths();
      writeFileSync(paths.songsPath, JSON.stringify(this.localSongs), "utf-8");
      writeFileSync(paths.artsPath, JSON.stringify(this.localSongsArts), "utf-8");
      console.log(`[LocalFiles] Saved ${this.localSongs.length} local songs to cache.`);
    } catch (e) {
      console.error("[LocalFiles] Failed to save local songs to cache:", e);
    }
  }

  static async scanLibrary(folders?: string[]) {
    // Load from cache on first scan for instant UI updates
    ...
    // Run the actual file scanner in the background
    this.runScannerAsync(folders).catch(() => {});

    return this.localSongs;
  }

  static async runScannerAsync(folders?: string[]) {
    try {
      if (!folders) folders = utils.getStoreValue("libraryPrefs.localPaths");
        ...
        try {
          ...
          if (metadata != null) {
            ...
            if (this.localSongs.length === 0 && numid % 10 === 0) {
              // send partial updates on first scan
      ...
      // Check if scanned results are different from cached
      const hasChanges = JSON.stringify(metadatalist) !== JSON.stringify(this.localSongs);
      if (hasChanges || this.localSongs.length === 0) {
        this.localSongs = metadatalist;
        this.localSongsArts = metadatalistart;
        this.saveToCache();
        const { BrowserWindow } = await import("../../base/browserwindow.js");
        if (BrowserWindow?.win) {
          BrowserWindow.win.webContents.send("getUpdatedLocalList", this.localSongs);
        }
      }
    } catch (err) {
      console.error("[LocalFiles] Background scan failed:", err);
    }
  }
