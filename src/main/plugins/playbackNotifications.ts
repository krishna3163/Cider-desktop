...
export default class playbackNotifications {
  ...
  createNotification(a: any): void {
    ...
    const playpauseText = this._utils.getLocale(this._utils.getStoreValue("general.language"), "term.playpause") || "Play/Pause";
    const nextText = this._utils.getLocale(this._utils.getStoreValue("general.language"), "term.next") || "Next";
                ...
                <toast>
                    ...
                    <visual>
                        <binding template="ToastGeneric">
                            <text>${escapeXml(a?.name)}</text>
                            <text>${escapeXml(a?.artistName)}</text>
                            <text>${escapeXml(a?.albumName)}</text>
                            <text placement="attribution">Cider Player</text>
                            <image placement="appLogoOverride" hint-crop="circle" src="${join(app.getPath("temp"), `${a.songId}-${a.artwork.url.split("/").pop()}`)}" />
                        </binding>
                    </visual>
                    <actions>
                        <action content="${escapeXml(playpauseText)}" activationType="protocol" arguments="cider://playpause/"/>
                        <action content="${escapeXml(nextText)}" activationType="protocol" arguments="cider://nextitem/"/>
                    </actions>
                </toast>`,
    });
